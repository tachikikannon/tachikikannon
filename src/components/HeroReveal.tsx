...
const READ_PAUSE_MS = 4000     // 小見出し（寺名）が出そろってから写真が現れるまでの余韻
const FINAL_PAUSE_MS = 3000    // 写真が現れてから、見出し類が消えて最終コンテンツに切り替わるまでの間
const FADE_MS = 700            // 見出し類のフェードアウト／最終コンテンツのフェードインにかかる時間
...
export default function HeroReveal({
  ...
  const headingGroupVisible = phase !== 'final' // 見出し・英字ラベルは最終フェーズで消える
  ...
  return (
    <section className={`relative flex items-center justify-center overflow-hidden bg-white ${className ?? ''}`}>
      ...
      {/* 見出し類・最終コンテンツを同じ位置に重ねてクロスフェードさせるための土台 */}
      <div className="relative w-full min-h-[280px] sm:min-h-[340px] md:min-h-[420px] lg:min-h-[480px] flex items-center justify-center px-4">
        <div
          className="absolute inset-0 flex flex-col items-center justify-center text-center px-4 transition-opacity ease-out"
          style={{ opacity: headingGroupVisible ? 1 : 0, transitionDuration: `${FADE_MS}ms` }}
        >
          <p
            className="text-xs tracking-[0.3em] mb-4"
            style={{
              opacity: textActive ? 1 : 0,
              color: imageActive || finalVisible ? eyebrowLightColor : eyebrowDarkColor,
                  ...
                  return (
                    <span
                      ...
                      style={{
                        ...
                        color: imageActive || finalVisible ? lightColor : darkColor,
        ...
        <div
          ...
          style={{
            ...
            transitionDuration: `${FADE_MS}ms`,
            transitionDelay: finalVisible ? '250ms' : '0ms',
            filter: photoVisible ? 'drop-shadow(0 2px 10px rgba(0,0,0,0.35))' : 'none',
          }}
        >
          {children}
        </div>
