...
export default function YakushikoApplyForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    await fetch('/api/notify/contact', {
      ...
      body: JSON.stringify({ name: form.name, email: form.email, subject: '【8月8日】薬師講大祭・採灯大護摩供 御札申し込み', message }),
    }).catch(() => {})
    setStatus('done')
  }
    ...
    <main className="min-h-screen pt-24 flex items-center justify-center px-4">
      <div className="text-center max-w-sm">
        ...
        <p className="text-gray-600 text-sm leading-relaxed mb-2">
          8月8日 薬師講大祭・採灯大護摩供の<br />御札申し込みを承りました。
        </p>
    ...
    <main className="pt-16 pb-16">
      <div className="bg-onsenji/5 px-4 py-2 text-xs text-gray-400">
        <div className="max-w-xl mx-auto">
          <Link href="/onsenji">ホーム</Link> &gt; <Link href="/onsenji/events">年間行事</Link> &gt; <Link href="/onsenji/events/yakushiko">薬師講大祭</Link> &gt; 申し込み
        </div>
      </div>
      ...
      <section className="bg-onsenji py-14 text-center relative overflow-hidden">
        ...
        <p className="text-white/60 text-sm mt-2 relative">薬師講大祭・採灯大護摩供（8月8日）</p>
      </section>
      ...
      <div className="max-w-xl mx-auto px-4 py-10">
        <div className="bg-onsenji/5 border border-onsenji/10 rounded-xl p-4 mb-6 flex gap-4 items-center">
          <div className="w-14 h-14 rounded-xl bg-onsenji flex flex-col items-center justify-center flex-shrink-0">
            <span className="text-[#7ec8a4] text-xs">8月</span>
            <span className="text-white font-bold text-lg leading-none">8</span>
          </div>
          <div>
            <p className="font-serif text-onsenji font-medium">薬師講大祭・採灯大護摩供</p>
          ...
          <div className="border-t border-gray-200 pt-5">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input required className="admin-input" placeholder="例：病気平癒、健康増進、延命長寿..." value={form.wish1} onChange={set('wish1')} />
              </div>
              <div>
                <label className="admin-label">願い事 2つ目 <span className="text-gray-400 text-xs">（任意）</span></label>
                <input className="admin-input" placeholder="例：家内安全、開運招福..." value={form.wish2} onChange={set('wish2')} />
              </div>
            </div>
          </div>
