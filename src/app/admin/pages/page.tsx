...
const SECTIONS: Section[] = [
  ...
  },
  {
    ...
    fields: [
      ...
      {
        key: 'jyuzu_stones', label: '選べる珠：天然石', type: 'list',
        listFields: [{ key: 'image', label: '写真', image: true }, { key: 'name', label: '石の名前' }, { key: 'desc', label: '説明', multiline: true }],
        defaultValue: J([
      ...
      {
        key: 'jyuzu_woods', label: '選べる珠：天然木', type: 'list',
        listFields: [{ key: 'image', label: '写真', image: true }, { key: 'name', label: '木の名前' }, { key: 'desc', label: '説明', multiline: true }],
        defaultValue: J([
  ...
  },
  {
    ...
    fields: [
      ...
      { key: 'funazento_info_fee', label: '参加費（カード表示）', defaultValue: '5,000円（子供4,000円）', translatable: true, defaultValueEn: '¥5,000 (¥4,000 for children)' },
      ...
      { key: 'funazento_apply_notice_fee', label: '申込フォーム 注意事項（💰 参加費）', multiline: true, defaultValue: '参加費は大人（中学生以上）御札付き5,000円、子供（小学生以下）御札付き4,000円です。当日ご参加されない場合は、お札のみ3,000円を頂戴いたします。', translatable: true, defaultValueEn: 'The participation fee is ¥5,000 for adults (junior high school age and older, ofuda included) and ¥4,000 for children (elementary school age and younger, ofuda included). If you do not attend in person, an ofuda-only fee of ¥3,000 applies.' },
      { key: 'funazento_apply_notice_shipping', label: '申込フォーム 注意事項（🚚 代金引換の送料）', multiline: true, defaultValue: '当日ご参加されない方のお札は代金引換（代引き）にて郵送いたします。代表者様のご住所へまとめて発送する場合、送料は1〜5件で1,000円（6件以上は5件ごとに1,000円加算）。代表者様以外のご住所へ発送する場合は、1件につき1,000円です。', translatable: true, defaultValueEn: 'For those not attending in person, the ofuda talisman is shipped by cash on delivery. When shipped together to the representative’s address, shipping is ¥1,000 for 1–5 items (an additional ¥1,000 per every 5 items beyond that). Shipping to an address other than the representative’s is ¥1,000 per item.' },
