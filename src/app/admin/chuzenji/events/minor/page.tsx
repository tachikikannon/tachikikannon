...
export default function AdminMinorEvents() {
  ...
  async function load() {
    const { data } = await supabase.from('minor_events').select('*')
  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-serif text-navy">こまごました行事</h1>
          <p className="text-xs text-gray-400 mt-1">年間行事ページ（観音講・船禅頂・正月元旦以外）の追加行事です。専用ページは持たず、一覧のカードと簡単な詳細ページのみになります。</p>
        </div>
        <button onClick={() => openEditor({ title:'', month_label:'', date_label:'', time_label:'', desc_text:'', sort_order: 0, is_published:false })} className="btn-primary text-sm px-4 py-2">＋ 行事を追加</button>
      </div>
            ...
            {list.map(ev => (
              <tr key={ev.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3 flex gap-2 justify-end">
                  <button onClick={() => openEditor(ev)} className="text-navy hover:underline text-xs">編集</button>
