...
export default function OnsenjiPhotoSelector({ photos }: { photos: Media[] }) {
  const t = useTranslations('onsenjiPhotos')
    ...
    <>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-5 pb-20">
        {photos.map((item, i) => {
          ...
          return (
            <div key={item.id}
              className={`bg-white rounded-xl shadow-sm overflow-hidden border-2 transition-colors ${isSelected ? 'border-onsenji' : 'border-transparent'}`}>
              ...
              <div className="p-3">
                ...
                <label className={`flex items-center justify-center gap-1.5 text-xs px-3 py-2 rounded-full border cursor-pointer transition-colors
                  ${isSelected ? 'bg-onsenji text-white border-onsenji' : 'bg-white text-onsenji border-onsenji hover:bg-onsenji/5'}`}>
      ...
      <div className="fixed bottom-0 inset-x-0 bg-white border-t border-gray-200 shadow-[0_-2px_10px_rgba(0,0,0,0.05)] z-40">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
          ...
          {selected.length > 0 ? (
            <Link href={`/onsenji/apply?category=${encodeURIComponent('写真使用・貸出し許可申請')}&photo=${encodeURIComponent(selected.join(','))}`}
              className="px-6 py-2.5 bg-onsenji text-white text-sm rounded-full hover:bg-onsenji/80 transition-colors whitespace-nowrap">
              {t('applySelectedCta')}
            </Link>
          ) : (
