...
export default function OnsenjiApplyForm() {
  const supabase = createClient()
  const searchParams = useSearchParams()
  const t = useTranslations('onsenjiApply')
    ...
    <main className="min-h-screen pt-24 pb-16 px-4">
      <div className="max-w-lg mx-auto">
        ...
        <div className="flex items-center justify-between mb-3">
          <h2 className="font-serif text-onsenji text-base">{t('doneSummaryHeading')}</h2>
  ...
  return (
    <main className="pt-24 pb-16 px-4">
      <div className="max-w-lg mx-auto">
        <nav className="text-xs text-gray-400 mb-6"><Link href="/onsenji">{tc('breadcrumbHome')}</Link> &gt; {t('title')}</nav>
        <h1 className="text-3xl font-serif text-onsenji mb-1">{t('title')}</h1>
        <p className="text-gray-500 text-sm mb-8">{t('intro')}</p>

        <div className="mb-8 p-5 bg-onsenji/5 rounded-xl flex flex-col sm:flex-row items-center gap-4">
          <div className="flex-1">
            <p className="font-serif text-onsenji text-sm mb-1">{t('photoBannerTitle')}</p>
            <p className="text-xs text-gray-600 leading-relaxed">{t('photoBannerText')}</p>
          </div>
          <Link href="/onsenji/photos" className="flex-shrink-0 px-5 py-2.5 bg-onsenji text-white text-xs rounded-full hover:bg-onsenji/80 transition-colors whitespace-nowrap">
            {t('photoBannerCta')}
          </Link>
        </div>
          ...
          <form onSubmit={goToConfirm} className="space-y-5">
            ...
            {(isGroupReservation || isFeeReduction) && (
              <div className="space-y-5 border-t border-gray-200 pt-6">
                <h2 className="font-serif text-onsenji text-lg">
            ...
            {isPress && (
              <div className="space-y-5 border-t border-gray-200 pt-6">
                <h2 className="font-serif text-onsenji text-lg">{t('sectionMediaHeading')}</h2>
                <div>
                ...
                <h2 className="font-serif text-onsenji text-lg pt-2">{t('sectionInterviewHeading')}</h2>
                <div>
            ...
            <div className="border-t border-gray-200 pt-6">
              ...
              {form.attachment_filename ? (
                <div className="flex items-center gap-3 bg-onsenji/5 rounded-lg px-4 py-3 text-sm">
                  <span className="text-onsenji">✓ {form.attachment_filename}</span>
            ...
            <button type="submit" disabled={uploading}
              className="w-full py-3 bg-onsenji text-white rounded-full font-medium hover:bg-onsenji/80 transition-colors text-sm text-center disabled:opacity-50">
              {t('goToConfirm')}
            </button>
          </form>
        )}
          ...
          <div className="space-y-6">
            ...
            <div className="flex gap-3">
              <button type="button" onClick={backToInput} disabled={submitting}
                className="flex-1 border border-onsenji text-onsenji rounded-full py-3 text-sm hover:bg-onsenji/5 transition-colors disabled:opacity-50">
                {t('backButton')}
              </button>
              <button type="button" onClick={handleSubmit} disabled={submitting}
                className="flex-1 py-3 bg-onsenji text-white rounded-full font-medium hover:bg-onsenji/80 transition-colors text-sm text-center disabled:opacity-50">
        ...
        <div className="mt-10 p-5 bg-onsenji/5 rounded-xl text-sm text-gray-600">
          <p className="font-medium text-onsenji mb-2">{t('phoneContactTitle')}</p>
          <p>TEL：0288-55-0013</p>
          <p className="text-xs text-gray-400 mt-1">{t('phoneContactHours')}</p>
        </div>
      </div>
    </main>
  )
