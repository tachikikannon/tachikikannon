...
const DEFAULTS: Record<string, string> = {
  about_fee_adult: '500円', about_fee_child: '200円',
  about_hours_peak: '午前8時〜午後5時', about_hours_shoulder: '午前8時〜午後4時', about_hours_winter: '午前8時30分〜午後3時30分',
  about_flow: JSON.stringify(DEFAULT_FLOW),
  about_grant_text: 'お守り・御朱印などの授与品は、御朱印所・五大堂・本堂の3か所でお受けいただけます。場所によって授与しているお守りが異なりますので、各所でぜひご覧ください。',
  about_notes: JSON.stringify(DEFAULT_NOTES),
}
...
export default async function AboutPage() {
  ...
  const flow  = pj<typeof DEFAULT_FLOW>(c.about_flow, DEFAULT_FLOW)
  const notes = pj<typeof DEFAULT_NOTES>(c.about_notes, DEFAULT_NOTES)
    ...
    <>
      ...
      <main className="pt-16">
        <div className="bg-cream-alt px-4 py-2 text-xs text-gray-400">
          <div className="max-w-5xl mx-auto"><Link href="/">ホーム</Link> &gt; 参拝について</div>
        </div>
        ...
        <div className="max-w-3xl mx-auto px-4 py-12 space-y-16">
          ...
          <section id="flow">
            ...
            <div className="mt-8 bg-cream-alt rounded-xl p-5 border-l-4 border-gold">
              ...
              <p className="text-sm text-gray-600 leading-relaxed">{c.about_grant_text}</p>
            </div>
          </section>
          <section id="notes">
            ...
            <ul className="space-y-3">
              {notes.map(({ text }, i) => (
                <li key={i} className="flex gap-3 bg-white rounded-lg px-4 py-3 shadow-sm border-l-4 border-gold text-sm text-gray-700 leading-relaxed">{text}</li>
              ))}
            </ul>
          </section>
          <section>
